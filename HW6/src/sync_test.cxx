...
__kernel void kernel_1_test(__global int *pDest, __global int *pSrc1,
                           __global int *pSrc2)
{
...
__kernel void kernel_2_test(__global int *pDest, __global int *pSrc1,
                           __global int *pSrc2)
{
    int index = get_global_id(0);
...
int main()
{
    ...
    cl_handler<cl_program> program{clCreateProgramWithSource(
        context.get(), 1, &kernelSource, nullptr, nullptr)};
    clBuildProgram(program.get(), 1, &device_id, nullptr, nullptr, nullptr);
    cl_handler<cl_kernel> kernel_1{
        clCreateKernel(program.get(), "kernel_1_test", nullptr)};
    set_kernel_args(kernel_1.get(), destMemObj.get_ptr(), src1MemObj.get_ptr(),
                    src2MemObj.get_ptr());
    std::size_t maxWorkGroupSize = 0;
    clGetDeviceInfo(device_id, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                    sizeof(maxWorkGroupSize), &maxWorkGroupSize, nullptr);

    clWaitForEvents(2, events->get_ptr());

    events[0].reset();
    events[1].reset();
    clEnqueueNDRangeKernel(command_queue.get(), kernel_1.get(), 1, nullptr,
                           &ELE_NUMS, &maxWorkGroupSize, 0, nullptr,
                           events[0].get_ptr());
    cl_handler<cl_kernel> kernel_2{
        clCreateKernel(program.get(), "kernel_2_test", nullptr)};
    set_kernel_args(kernel_2.get(), destMemObj.get_ptr(), src1MemObj.get_ptr(),
                    src2MemObj.get_ptr());
    clEnqueueNDRangeKernel(command_queue.get(), kernel_2.get(), 1, nullptr,
