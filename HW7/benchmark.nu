...
def benchmark [len: int] {
    let $cpu_result = 1..10 | each { |item|
        let result = xmake r convolution_cpu $len --benchmark | lines
    ...
    let $avx2_result = 1..10 | each { |item|
        let result = xmake r convolution_cpu_optimized $len --benchmark | lines
    ...
    let $opencl_result = 1..10 | each { |item|
        let result = xmake r convolution_opencl $len --benchmark | lines
        let int8_time = $result.0 | into float
        let int16_time = $result.1 | into float
        let int32_time = $result.2 | into float
    ...
    let $opencl_optimized_result = 1..10 | each { |item|
        let result = xmake r convolution_opencl_optimized $len --benchmark | lines
        let int8_time = $result.0 | into float
        let int16_time = $result.1 | into float
        let int32_time = $result.2 | into float
        let f16_time = $result.3 | into float
    ...
    print $"AVX2 i16 Time: ($avx2_i16_time), i32: ($avx2_i32_time), f32: ($avx2_f32_time), f64: ($avx2_f64_time)"
